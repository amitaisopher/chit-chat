...
async function buildServer() {
    ...
    app.io.on('connection', async (io) => {
        ...
        const incResults = await publisher.incr(CONNECTION_COUNT_KEY)
        await publisher.publish(CONNECTION_COUNT_UPDATED_CHANNEL, String(incResults))
        io.on("disconnect", async () => {
            connectedClients--
            console.log('client disconnected')
            const decResults = await publisher.decr(CONNECTION_COUNT_KEY)
            await publisher.publish(CONNECTION_COUNT_UPDATED_CHANNEL, String(decResults))
        })
    ...
    subscriber.subscribe(CONNECTION_COUNT_UPDATED_CHANNEL, (err, count) => {
        ...
        console.log(`Client subscribed to ${CONNECTION_COUNT_UPDATED_CHANNEL} channel`)
    })
